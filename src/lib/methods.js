...
export async function depositCAP(amount) {
	...
	try {
		...
		hideModal();
		amplitude.getInstance().logEvent('Pool Deposit ONA', {amount});
	} catch(e) {
		showToast(e);
		return e;
	}
...
export async function withdrawCAP(amount) {
	...
	try {
		...
		hideModal();
		amplitude.getInstance().logEvent('Pool Withdraw ONA', {amount});
	} catch(e) {
		showToast(e);
		return e;
	}
...
export async function collectCAPReward(currencyLabel) {
	...
	try {
		...
		amplitude.getInstance().logEvent('Pool Collect ONA', {currencyLabel});
	} catch(e) {
		showToast(e);
		return e;
	}
